...
std::string Transpiler::generateExpression(Expression *expr,
  ...
  if (auto e = dynamic_cast<LambdaExpression*>(expr)) {
      ...
      for (size_t i = 0; i < e->parameters.size(); ++i) {
          functionsCode << "    Value sp_" << e->parameters[i] << " = args[" << i << "];\n";
      }
  ...
  if (auto e = dynamic_cast<IfExpression*>(expr)) {
      std::string code = "( (" + generateExpression(e->condition.get(), outStream) + ".asBoolean()) ? ";
      code += "(" + generateExpression(e->thenBranch.get(), outStream) + ") : ";
      if (e->elseBranch) {
          code += "(" + generateExpression(e->elseBranch.get(), outStream) + ") ";
      } else {
          code += "Value(Type::NULL_VAL) ";
      }
      code += ")";
      return code;
  }
...
void Transpiler::generateStatement(Statement *stmt,
  ...
  if (auto funcDecl = std::get_if<FunctionDeclaration>(&*stmt)) {
    ...
    for (size_t i = 0; i < funcDecl->parameters.size(); ++i) {
      functionsCode << "    Value sp_" << funcDecl->parameters[i] << " = args["
                    << i << "];\n";
    }
    ...
    if (inFunction) {
      ...
      for (const auto& method : classDecl->methods) {
          ...
          for (size_t i = 0; i < method.parameters.size(); ++i) {
              functionsCode << "    Value sp_" << method.parameters[i] << " = args[" << i + 1 << "];\n";
          }
