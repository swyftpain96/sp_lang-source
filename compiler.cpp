...
CompilationResult Compiler::compile(const std::string& name, const std::vector<Statement>& statements, int localCount) {
    ...
    result.allStrings = std::move(allStrings);
    result.allBigInts = std::move(allBigInts);
    return result;
}
    ...
    if (std::holds_alternative<VariableDeclaration>(stmt)) {
        ...
        if (isNative) {
            for (const auto& [exp, alias] : useStmt.namedImports) {
                ...
                allStrings.push_back(modNamePtr);
                ...
                allStrings.push_back(symNamePtr);
        ...
        if (useStmt.isNamed) {
            ...
            for (const auto& [exp, alias] : useStmt.namedImports) {
                ...
                allStrings.push_back(fullNamePtr);
        ...
        for (const auto& prop : decl.properties) {
            ...
            allStrings.push_back(propNamePtr);
        ...
        for (const auto& method : decl.methods) {
            ...
            allStrings.push_back(methodNamePtr);
