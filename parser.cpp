...
Statement Parser::variableDeclaration(bool isMutable) {
  ...
  auto value = expression();
  return addLine(VariableDeclaration{name, std::move(value), -1, isMutable}, line);
}
...
Statement Parser::functionDeclaration() {
  ...
  return addLine(FunctionDeclaration{name, params, std::move(body), 0, -1, hasRest}, line);
}
...
Statement Parser::classDeclaration(bool isAbstract) {
  ...
  while (!check(TokenType::RBRACE) && !check(TokenType::END)) {
    ...
    if (match(TokenType::SET) || match(TokenType::VAR)) {
      ...
      properties.push_back({propName, std::move(initializer), Value(), isPrivate, isReadonly, memberLine});
    } else if (match(TokenType::DEFINE)) {
      // Method
...
Statement Parser::exportStatement() {
  ...
  throw std::runtime_error("Expected variable or function declaration after export");
}
...
std::shared_ptr<Expression> Parser::primary() {
  ...
  if (match(TokenType::NUMBER)) {
    ...
    if (isLambda) {
        ...
        expr = addLine(std::make_shared<LambdaExpression>(params, body, hasRest), line);
    } else {
        expr = expression();
        if (!match(TokenType::RPAREN)) {
...
std::shared_ptr<Expression> Parser::processExpr() {
    ...
    if (match(TokenType::SPAWN)) {
        ...
        throw std::runtime_error("Expected run or spawn after process.");
    }
